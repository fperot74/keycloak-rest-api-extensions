...
del_configuration()
{
  echo "Removing configuration $1 from configuration file $CONF_FILE)"
  sed -i "/^$1=/d" ${CONF_FILE}
}
...
cleanup()
{
    ...
    echo "cleanup..."
    add_configuration spi.api.enabled
    add_configuration spi.api.terms-of-use-acceptance-delay-days
