...
public class CtExecuteActionsActionTokenHandler extends AbstractActionTokenHandler<CtExecuteActionsActionToken> {
    ...
    public Response handleToken(CtExecuteActionsActionToken token, ActionTokenContext<CtExecuteActionsActionToken> tokenContext) {
        ...
        String emailClaim = (String) token.getOtherClaims().get(CtExecuteActionsActionToken.CLAIM_EMAIL);
        setEmailVerified(user, emailClaim);
    ...
    private void setEmailVerified(UserModel user, String emailClaim) {
        if (StringUtils.isBlank(emailClaim)) {
            // Email to validate is not present in the token so we are basically checking the main email (user::getEmail)
            // If we reached this point, the email is valid
