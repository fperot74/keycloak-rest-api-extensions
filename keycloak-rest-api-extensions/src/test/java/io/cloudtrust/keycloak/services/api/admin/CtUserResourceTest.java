...
public class CtUserResourceTest extends AbstractRestApiModuleTest {
    ...
    private void createDummyRealm(String realm, String theme) {
        ...
        getKeycloakAdminClient().realms().create(realmRepresentation);
    }

    @After
    public void deleteDummyRealms() {
    	Keycloak kcAdminClient = getKeycloakAdminClient();
    	kcAdminClient.realm("dummy1").remove();
    	kcAdminClient.realm("dummy2").remove();
    }
    ...
    private void successCase(String queryParams) throws IOException, URISyntaxException, MessagingException {
    	RealmResource testRealm = this.getRealm("test");
